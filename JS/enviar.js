...
// Mostra una alerta amb les dades preparades per a l'enviament a la BBDD
function showDbDataAlert(extractedData) {
  if (!extractedData) {
    alert(
      "No es poden enviar dades a la BBDD: No s'ha processat cap document o l'extracció ha fallat."
    );
    return;
  }
  const dbObject = prepareDataForDB(extractedData);
  const jsonPayload = JSON.stringify(dbObject, null, 2);

// Mostrar l'alerta amb les dades en format JSON
  alert(`Dades a enviar a BBDD (JSON):\n${jsonPayload}`);
  console.log("JSON per BBDD:", dbObject);
}
...
async function processFile( 
  ...
  if (file && file.type === "application/pdf") {
    ...
    try {
      ...
      sendToDbButton.onclick = () => showDbDataAlert(currentExtractedData);
      return fileURL; 
    } catch (error) {
